...

{
...

    {
        ...
        $this->createlogs(@$_SESSION['login']['username'], 'insert', $tableName, $this->pdo()->lastInsertId());
...

    {
        ...
        if (strpos($this->query, 'delete') !== false) {
            ...
            $this->createlogs(@$_SESSION['login']['username'], 'query', "tables", $queryLogs);
        }
...

    {
        ...
        $this->createlogs(@$_SESSION['login']['username'], 'update', $tableName, $wherelogs);
    ...
    public function createTable()
    {
        ...
        include_once  _lib . 'langAttr.php';

        $sql = "";

        for ($i = 0; $i < count($langAttr); $i++) {

            $table = $langAttr[$i]["name"];

            $this->query = "CREATE TABLE $this->prefix$table (";

            for ($j = 0; $j < count($langAttr[$i]['value']); $j++) {

                $column = $langAttr[$i]['value'][$j]['col'];

                $type = $langAttr[$i]['value'][$j]['type'];

                $sql .= "$column $type COLLATE utf8_unicode_ci NOT NULL,";
            }
    ...
    /*table exists*/
    public function if_table_exists()
    {

        $this->query = "SHOW TABLES";

        $row = $this->rawQuery($this->query);
    ...
    public function createlogs($user = 'Unnamed', $act, $com, $id)
    {
        ...
        $ip_user = $this->getClientAddressIp();
