<?php
    class statitis{
        ...
        public function counterOn(){         
            ...
            $t = $this->cache->getCache("select max(id) as total from #_counters",'fetch',1800);
        ...
        public function getCounter(){
            ...
            $todayrc = $this->cache->getCache("select count(*) as todayrecord from #_counters where tm>'$daystart'",'fetch',1800);

            $result['today']     =    $todayrc['todayrecord'];

            $yesrec = $this->cache->getCache("select count(*) as yesterdayrec from #_counters where tm>'$yesterdaystart' and tm<'$daystart'",'fetch',1800);
            
            $result['yesterday']     =    $yesrec['yesterdayrec']; 

            $weekrec = $this->cache->getCache("select count(*) as weekrec from #_counters where tm>='$weekstart'",'fetch',1800);

            $result['week']     =    $weekrec['weekrec']; 

            $monthrec = $this->cache->getCache("select count(*) as monthrec from #_counters where tm>='$monthstart'",'fetch',1800);

            $result['month']     =    $monthrec['monthrec'];  

            $totalrec = $this->cache->getCache("select max(id) as totalrec from #_counters",'fetch',1800);
        ...
        public function statusOnline(){
            ...
            if($i<$result['dangxem']){
                ...
                $id_insert = $this->_d->insert('onlines', $data);
