...
class Upload {
     ...
     *
     * By default, the language is english (vn_VN)
     ...
     */
    function  __construct($file, $lang = 'vn_VN')  {
    ...
    function process($server_path = null) {
        ...
        if ($this->processed) {
            ...
            if ($this->no_script) {
                ...
                if ((((substr($this->file_src_mime, 0, 5) == 'text/' && $this->file_src_mime != 'text/rtf') || strpos($this->file_src_mime, 'javascript') !== false)  && (substr($file_src_name, -4) != '.txt') && (substr($file_src_name, -4) != '.svg'))
    ...
    /* cũ function process($server_path = null) {
        $this->error        = '';
        $this->processed    = true;
        $return_mode        = false;
        $return_content     = null;
