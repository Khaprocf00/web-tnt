...
$(document).ready(function() {
    ...
    $('.js-change-brand').change(function(){
...
function get_main_list($pid,$id_brand)

{

  global $db,$items;

  $sql="select * from table_baiviet_list where type='menu' order by stt asc";

  $stmt=$db->rawQuery($sql);

  $str='

    <select data-id="'.$pid.'" name="id_brand" class="main_select js-change-brand">

    <option value="">Chọn menu</option>';

    foreach($stmt as $k => $v){

      $selected=($v["id"]==$id_brand) ? "selected" : "";
                ...
                <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
                    <thead>
                        <tr>
                            ...
                            <?php } ?>
                            <?php if($GLOBAL_LEVEL1[$com][$type]['id_brand']==true){ ?>
                            <td class="sortCol">
                                <div>Chọn menu<span></span></div>
                            </td>
                            <?php } ?>
                            <td class="sortCol">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td align="center" class="title_name_data">
                                <?=get_main_list($items[$i]["id"],$items[$i]["id_brand"])?>
                            </td>
                            <?php } ?>
                            <td align="center" class="title_name_data">
                                <a href="index.html?com=baiviet&act=edit_list&tbl=<?=$_GET['tbl']?>&id=<?=$items[$i]['id']?><?php if($_GET['type']!='') echo'&type='. $_GET['type'];?><?php if($_GET['page']!='') echo'&page='. $_GET['page'];?>"
