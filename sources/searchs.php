...
$sql = "SELECT *,tenkhongdau_$lang as tenkhongdau from #_baiviet where type ='dich-vu' {$where} {$order_by} " . $limit;

$tintuc = $db->rawQuery($sql);

$sqlq = "SELECT COUNT(*) as `numb` from #_baiviet where type ='dich-vu' {$where} {$order_by}";
...
if ($func->isAjax()) {
    ...
    echo json_encode([
        ...
        'paging' => $layouts->getLoadMoreInPage($total_paging, $per_page, $page, 'sản phẩm')

    ]);
} else {

    $url = $func->getCurrentPageURL_SEARCH();
