...
if (!empty($idl)) {
    ...
    if ($row_list) {
        ...
        $row_cat = $db->rawQuery("select id,ten_$lang as ten,tenkhongdau_$lang as tenkhongdau,photo,id_list,type,photo from #_baiviet_cat where hienthi=1 and type=? and id_list=? order by stt asc", array($type, $row_list["id"]));
        ...
        if ($func->isAjax()) {
            ...
            echo json_encode([
                ...
                'html' => $layouts->getLayoutProduct($tintuc, 'h3'),
        ...
        // if($type=='san-pham' && count($row_cat)>0){
...
if (!empty($idc)) {
    ...
    if ($row_cat) {
        ...
        $row_item = $db->rawQuery("select id,id_list,ten_$lang, tenkhongdau_$lang as tenkhongdau,photo,type from #_baiviet_item where hienthi=1 and id_cat=? and type=? order by stt desc,id desc", array($row_cat['id'], $type));
        ...
        // if($type=='san-pham' && count($row_item)>0){
...
if (empty($id) && empty($idl) && empty($idc) && empty($idi) && empty($ids)) {
    ...
    if ($func->isAjax()) {
        ...
        switch ($com) {
            ...
            case 'du-an':
                ...
                $getTemplate = $layouts->getLayoutProduct($tintuc, 'h3');
            ...
            case 'blogs':
                ...
                $getTemplate = $func->gridTemplateNews($tintuc, 'h3');
            ...
            default:
                ...
                $getTemplate = $func->gridTemplateDefault($tintuc, 'starceiling', 'col l-6 m-12 c-12 mb20 mb-m-10', 'h3', '321', '233');
    ...
    // if($type=='san-pham' && count($row_list)>0){
